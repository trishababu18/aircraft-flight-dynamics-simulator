...
subroutine solve_linear_system(Ain, bin, x)
  !! Convenience wrapper: solves Ain * x = bin using lu_solve
  implicit none
  real, intent(in)  :: Ain(:,:)
  real, intent(in)  :: bin(:)
  real, intent(out) :: x(:)
  ...
  integer :: n
  real, allocatable :: A(:,:), b(:), x_local(:)

  n = size(bin)
  if (size(Ain,1) /= n .or. size(Ain,2) /= n .or. size(x) /= n) then
    write(*,*) 'solve_linear_system: dimension mismatch'
    stop
  end if
  ...
  call lu_solve(n, A, b, x_local)

  x = x_local

  deallocate(A, b, x_local)
